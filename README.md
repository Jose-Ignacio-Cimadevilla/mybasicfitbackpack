...
For a high-level overview of the business observability requirements and architecture please go to [**Metrics description and architecture**](https://maersk-tools.atlassian.net/wiki/x/fYCYEys)
...
1. **Domain data** Raw source-of-truth or transactional application-generated data that can be used for **metric calculations**, as for instance: actual bunker consumption at vessel and leg level.
2. **Business metric** Calculated information based on one or more **domain datasets** with **calculation logic** applied, that can be readily used to provide insight for decision making in **dashboard**, as for instance actual bunker waste reduction.
3. **Metric calculation process** A data transformation logic or job that reads one or more **Domain datasets**, process them in a certain way, such as cleaning, filtering or applying formulas, and then writes the output into a 4. **Business metric** dataset that can be used for further processing or directly in a **Dashboard**.
5. **INDEX/Symphony dataset** Platform related data generated by INDEX tasks or Symphony workflow engine, as for instance task start and completion timestamps.
6. **Dashboard** A user-interface composite of **Business metrics** of various types, it provides an end-to-end view for a certain business domain together with meaningful INDEX platform information.
